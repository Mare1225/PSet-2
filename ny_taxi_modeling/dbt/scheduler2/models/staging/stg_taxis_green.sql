WITH source_data AS (
    SELECT * FROM {{ source('ny_taxi2', 'ny_taxi_green') }}
    ...
    WHERE
        ...
        AND "mta_tax" <= 60
        AND "tolls_amount" <= 60
        AND "improvement_surcharge" <= 60

        AND "lpep_pickup_datetime" > '2015-01-01'
        AND "lpep_pickup_datetime" < '2025-09-01'

        AND "lpep_dropoff_datetime" > '2015-01-01'
        AND "lpep_dropoff_datetime" < '2025-09-01'
),
...
renamed_and_cleaned AS (
    SELECT
        ...
        "lpep_pickup_datetime" AS pickup_datetime_utc,
        "lpep_dropoff_datetime" AS dropoff_datetime_utc,
