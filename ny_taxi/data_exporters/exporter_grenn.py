...
@data_exporter
def export_data_to_snowflake(df_green: pd.DataFrame, **kwargs) -> None:
    
    database   = get_secret_value('snowflake_db')
    schema     = get_secret_value('snowflake_schema')
    table_name_green = get_secret_value('snowflake_green_table')
    ...
    with Snowflake.with_config(ConfigFileLoader(config_path, config_profile)) as loader:
        for _, row in df_green.iterrows():
            ...
            try:
                ...
                print("No existe disponibilidad de Parquet")
